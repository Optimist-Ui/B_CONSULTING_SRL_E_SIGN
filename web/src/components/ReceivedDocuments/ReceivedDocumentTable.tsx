// src/components/ReceivedDocuments/ReceivedDocumentTable.tsx
...
interface ReceivedDocumentTableProps {
    documents: Document[];
    loading: boolean;
...
const ReceivedDocumentTable: React.FC<ReceivedDocumentTableProps> = ({ documents, loading, expandedRows, toggleExpansion, onDownload, onViewDetails }) => {
    ...
    const columns = useMemo<ColumnDef<Document, any>[]>(
        () => [
            ...
            // 🆕 NEW: Show user's roles in this document
            columnHelper.display({
                id: 'myRoles',
                header: t('receivedDocuments.headers.myRole'),
                cell: ({ row }) => {
                    const doc = row.original as any;
                    const roles = doc.currentUserRoles || [];
                    return (
                        <div className="flex flex-wrap gap-1">
                            {roles.map((role: string, idx: number) => (
                                <span key={idx} className="inline-block px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                                    {role}
                                </span>
                            ))}
                        </div>
            ...
            columnHelper.accessor('addedOn', {
                header: t('documentTable.headers.addedOn'),
            ...
            columnHelper.display({
                ...
                cell: ({ row }) => {
                    const doc = row.original as any;
                    const actionLink = doc.actionLink || `/package/${row.original.id}`;
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                            <Dropdown placement="bottom-end" btnClassName="p-1 rounded-full hover:bg-gray-200" button={<IconPlus className="w-5 h-5 text-gray-500" />}>
                                <ul className="min-w-[180px] bg-white shadow-lg rounded-md">
                                    <li>
                                        <button className="w-full flex items-center px-3 py-2 text-sm hover:bg-primary hover:text-white" onClick={() => window.open(actionLink, '_blank')}>
                                            <IconEye className="w-4 h-4 mr-2" />
                                            {t('receivedDocuments.actions.viewAndSign')}
                                        </button>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <button className="w-full flex items-center px-3 py-2 text-sm hover:bg-primary hover:text-white" onClick={() => onViewDetails(row.original.id)}>
                                            <IconEye className="w-4 h-4 mr-2" />
        ...
        [onDownload, onViewDetails, t, navigate]
    );
                        ...
                        <tr>
                            <td colSpan={columns.length} className="p-10 text-center">
                                <div className="flex flex-col items-center justify-center">
                                    ...
                                    <h3 className="text-lg font-semibold mb-2">{t('receivedDocuments.emptyState.title')}</h3>
                                    <p className="text-gray-500 text-sm">{t('receivedDocuments.emptyState.description')}</p>
                                </div>
                            </td>
                        </tr>
                    ) : (
