...
const formatDateTime = (dateString: string | 'N/A') => {
    if (!dateString || dateString === 'N/A') {
        return 'N/A';
    }
    try {
        const date = new Date(dateString);
        // For more robust parsing, especially across browsers, consider a library like date-fns if issues arise.
                        ...
                        {members.map((member, index) => {
                            ...
                            return (
                                <tr key={index} className="border-t border-gray-200">
                                    ...
                                    <td className="px-4 py-2 text-xs">{formatDateTime(lastUpdated)}</td>
                                </tr>
                            );
                        })}
                    </tbody>
...
const DocumentDetailsPanel: React.FC<DocumentDetailsPanelProps> = ({ formFillers, approvers, signers, receivers }) => {
    return (
