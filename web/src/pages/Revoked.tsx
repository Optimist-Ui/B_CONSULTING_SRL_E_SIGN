...
const revokedDocuments: RevokedDocument[] = [
    {
        id: 1,
        name: 'Project Contract.pdf',
        status: 'Revoked',
        addedOn: '2023-11-20',
        revokedDate: '2023-11-25',
        revokedBy: 'Admin Manager',
        revokedReason: 'Contract terms require revision',
        formFillers: ['Mark Wilson', 'Sarah Lee'],
        approvers: ['James Smith'],
        signers: ['Lisa Johnson'],
        receivers: ['Peter Brown'],
            ...
            formFillers: [
                {
                    ...
                    status: 'Revoked',
                    date: '2023-11-21',
                    revokedReason: 'Information needs updating'
                },
                {
                    name: 'Sarah Lee',
...
const Revoked = () => {
    ...
    const [filteredDocuments, setFilteredDocuments] = useState<RevokedDocument[]>(revokedDocuments);

    useEffect(() => {
        const filtered = revokedDocuments.filter((doc) => 
                        ...
                        {paginatedDocuments.map((doc) => (
                            <React.Fragment key={doc.id}>
                                <tr className="cursor-pointer hover:bg-primary/5" onClick={() => handleDocumentClick(doc.id)}>
                                    ...
                                    <td>{doc.addedOn}</td>
                                    <td>{doc.revokedDate}</td>
                                    <td>{doc.revokedBy}</td>
                                    <td>
                                        <span className="badge bg-danger">{doc.status}</span>
                                    </td>
                                        ...
                                        <td colSpan={8}>
                                            <div className="bg-gray-50 p-4">
                                                <div className="grid grid-cols-4 gap-4 mb-4 bg-white p-3 rounded shadow-sm">
                                                    ...
                                                    <div>
                                                        <span className="font-semibold">Revoked By:</span> {doc.revokedBy}
                                                    </div>
                                                    <div>
                                                        <span className="font-semibold">Reason:</span> {doc.revokedReason}
                                                    </div>
                                                </div>
                                                        ...
                                                        {Object.entries(doc.documentDetails).map(([role, participants]: [string, Participant[]]) =>
                                                            participants.map((participant: Participant, idx: number) => (
                                                                <tr key={`${role}-${idx}`} className="border-b">
                                                                    ...
                                                                    <td>
                                                                        <span className="badge bg-danger">{participant.status}</span>
                                                                    </td>
                                                                    <td>{participant.date}</td>
                                                                    <td>{participant.revokedReason || '-'}</td>
                                                                </tr>
                                                            ))
                                                        )}
                                                    </tbody>
